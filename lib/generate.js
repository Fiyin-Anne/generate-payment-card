...
module.exports = {
    generateCardNumber(data) {
        const { error, value } = fieldsValidation(data);

        
        if(error) throw new Error(error)

        const { user_digits } = value;
        ...
        if (value.user_digits.status && value.user_digits.position === 'startswith') {
            //todo: find a more effective way to identify brand by first 2 to 4 digits
            var first_digits = String(value.user_digits.digits).slice(0,2);
            ...
            card_brand = !value.card_brand || value.card_brand.length === 0 ? 'visa' : value.card_brand;
        }
        
        if (value.issuer) {
            card_brand_bins = card_bins.filter(bin => bin["BRAND"] === card_brand.toUpperCase() && bin["COMPANY_NAME"].startsWith(value.issuer.toUpperCase()));
        ...
        cvv_and_expiry_date = cvv_and_expiry(value.expires_in);
        ...
        return {
            ...
            balance: value.balance, // moved all balance logic to the validation layer
            issuer: issuer,
